  ...
  return (
    <svg viewBox="0 0 200 160" className="w-full h-full">
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => {
        ...
        return (
          <g key={i}>
            <line x1="100" y1="80" x2={(100 + Math.cos(angle) * radius).toFixed(6)} y2={(80 + Math.sin(angle) * radius).toFixed(6)} stroke="currentColor" strokeWidth="1" opacity="0.3">
              <animate attributeName="opacity" values="0.3;0.8;0.3" dur="2s" begin={`${i * 0.3}s`} repeatCount="indefinite" />
            </line>
            <circle cx={(100 + Math.cos(angle) * radius).toFixed(6)} cy={(80 + Math.sin(angle) * radius).toFixed(6)} r="6" fill="none" stroke="currentColor" strokeWidth="2">
...
function FeatureCard({ feature, index }: { feature: { number: string; title: string; description: string; visual: string }; index: number }) {
...
export function FeaturesSection() {
  ...
  const features = [
    { number: "01", title: t("features.f1.title"), description: t("features.f1.desc"), visual: "deploy" },
    { number: "02", title: t("features.f2.title"), description: t("features.f2.desc"), visual: "ai" },
    { number: "03", title: t("features.f3.title"), description: t("features.f3.desc"), visual: "collab" },
  ];
